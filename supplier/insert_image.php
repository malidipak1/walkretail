...
@chmod($p1, 0777);
	   ...
	   $name= $_FILES['userfile']['name'];
	     $newphrase = str_replace(' ','_' , $name);
         $uploadFile = $uploadDir.$newphrase;
...
<html>
<head>
<title>Select Image / Upload Image to Server</title>
...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <form action="insert_image.php?upload=yes" name="form1" method="post" enctype="multipart/form-data">
  <tr>
      <td valign="top"> 
        <!-------------------------------------------------------------------------->
        ...
        <table width="100%"  border="0" cellspacing="0" cellpadding="0">
          ...
          <tr> 
			...
			<table width="80%" border="0" align="center" cellspacing="0"  cellpadding="5">
                <tr > 
                  <td colspan="2"  align="center"><font color="#5b6c0c"><?php echo $msg;?></font></td>
                </tr>
                <tr > 
                  <td width="37%"  align="right"><font color="#000000">Select Image File :</font></td>
                  <td width="63%"  align="left"> <input type="file" name="userfile"  class="inputbox-boder"> 
                  </td>
                </tr>
                <tr> 
                  <td align="center"  colspan="2">
				   <input name="Submit"  style="width:150px;" type="submit" value="Upload Image" onClick="document.form1.pg.src='c002.gif';">
                  </td>
...
$title = "Image selection tools";// Title (may contain HTML tags)
...
$thbgcolor    = "#999999";         // Background color of table headers
$thfgcolor    = "#000000";         // Foreground color of table headers
$tdbgcolor    = "#FFFFFF";         // Background color of table cells
...
$actlinkcolor = "#000000";         // Active link color
...
$imagesdir = "pgimages";          // Images directory (must be located in base directory)
...
$basedir = "pgimages";
//@dirname(__FILE__);    // Base directory = local directory
...
$basevirtualdir = "pgimages";              // If you have set a custom $basedir AND $filelinks = true
...
$messages = array(
   ...
   "sch9"=>"Go to folder",
   "dwn1"=>"Download", // D(ownload action)
...
echo "<p><table border=0 cellspacing=2>";
...
// File list page
   ...
   if (!empty($files)) {
      echo "<table style='border:0px solid #999999;' cellspacing=1 width=100% bgcolor=#ccccc>";
      echo "<form action=$thisscript method=post name=listForm>";
      echo "<input name=act type=hidden value=''>";
      echo "<input name=subdir type=hidden value=\"$subdir\">";
      echo "<input name=sortby type=hidden value=$sortby>";
     // echo "<tr>";
     // echo "<td  height=0 class=tdcc></td>";
     // echo "<td  height=0 class=tdcc></td>";
     // echo "<td  height=0 class=tdcc></td>";
      //echo "<td  height=0 class=tdcc></td>";
     // echo "<td  height=0 class=tdcc></td>";
...
*/
         ...
         // File section
			...
			$f=substr(htmlspecialchars($file["name"]),strlen(htmlspecialchars($file["name"]))-3,strlen(htmlspecialchars($file["name"])));

    if($f=="jpg" || $f=="JPG" || $f=="GIF" || $f=="gif" || $f=="PNG" || $f=="png")
    {
           ...
           // echo "<td>&nbsp;</td>";
            echo "<td valign='middle' style='text-align:left;'>".(($file["link"]) ? "<i>" : "");
            if ($filelinks) {
			 list($width,$height)=getimagesize($file["path"]);
			//echo strlen(htmlspecialchars($file["name"]))-2;
			$path12=str_replace("//", "/",trim($file["path"]));
			   echo '<a href="#" onClick=ins("'.$path12.'","'.$width.'","'.$height.'","'.$_SERVER['HTTP_HOST'].'")>';
			   echo "<img src='".$path12."' border=0 width=50 height=50 alt='Insert this image on page'>&nbsp;&nbsp;&nbsp;&nbsp;";			   
			  echo "</a>"; 
			   if ($basevirtualdir == "") {
            ...
            if (($allowdownload) && ($subdir != $trashcan)) {
              ...
              echo"<a href='delete.php?act=delete&file=".rawurlencode($file["name"])."'>Delete</a>";
		    }
            echo "</td>";
            echo "</tr>";
      ...
      echo "<th colspan=$n>$nbdirsremove ".$messages["tab11remove"]." $nbfiles ".$messages["tab12"]." (".round($total/1024)." ".$messages["tab13"].")</th>";
      echo "</tr>";
...
<script language="JavaScript">
function ins(srcs,wid,hei,host)
{  
  //var dirs='projects/caliber';
 opener.document.myform.src.value="http://"+host+"<?php $y= $_SERVER['REQUEST_URI'];echo preg_replace('/insert_image\.php/', '', $y); ?>"+srcs;
  opener.document.myform.src.focus();
  self.close();
