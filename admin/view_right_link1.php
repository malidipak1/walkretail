...
$file_name			=	"view_right_link1.php"; // this is file name which is used during paging  , included at the bottom of the page
$paging_table_name	=	"notice";
include("paging/paging_query.inc.php");

############################################ END PAGING WITH PER PAGE       ##############################################
$sql_product		= 	"SELECT * FROM rightlink1 LIMIT $eu, $limit ";
...
{
	$sql_product		= 	"SELECT * FROM results where enrollment_no='".$_REQUEST['enroll_id']."' LIMIT $eu, $limit ";
	$result_product		=	mysql_query($sql_product);
	$num_rows_product	=	mysql_num_rows($result_product);
}*/
/*if(isset($_REQUEST['enroll_id']) && $_REQUEST['enroll_id']!='')
{
	$query2="SELECT * FROM results WHERE enrollment_id = '".$_REQUEST['enroll_id']."'";
...
{
	$sql=mysql_query("select * from rightlink1 ORDER BY rightlink1_id");
	$result=mysql_num_rows($sql);
}

if(isset($_REQUEST['action']) && $_REQUEST['action']=='delete')
{
	$sql	=	"DELETE FROM rightlink1 where rightlink1_id ='".$_REQUEST['rightlink1_id']."' ";
	mysql_query($sql);
	header("location:view_right_link1.php?action=view&message=Deleted successfully&action=view");
	exit;
}
################################################### END DELETE ########################
...
{
	$enroll_id 		=	$_REQUEST['rightlink1_id'];
...
<table width="100%" height="100%" align="center" cellpadding="0" cellspacing="0"  bordercolor="#5181BF">
  ...
  <tr>
                                    ...
                                    <td width="100%" align="left" valign="top" bgcolor="" class="red"><table width="100%" border="0"  align="center" cellpadding="0" cellspacing="0"  bordercolor="#111111">
                                      <tr>
                                        <td height="57" colspan="2" align="left"  valign="middle" class="head_ing">Manage Right Area</td>
                                      </tr>
                                      <tr>
	  	...
	  	<form name="search_product" method="post" action="view_right_link1.php?action=search_product">
...
<?php /*if(isset($_REQUEST['action']) && ($_REQUEST['action']=='search_product' || $_REQUEST['action']=='view') )
									{*/?>
										 <tr align="center"  bgcolor="#3270B4">									
                                          ...
                                          <td width="28%" bgcolor="#3c7701" class="white">Heading</td>
                                          <td width="56%" bgcolor="#3c7701" class="white">Details</td>
                        ...
                        Detail </b></td> -->
                                          ...
                                          <td width="8%" bgcolor="#3c7701" class="white">Edit</td>
			  ...
			  while($row_product=mysql_fetch_array($result_product))
				...
				?>
                                           <tr align="center">	
                                            ...
                                            <td width="28%" height="57" align="left" bgcolor="#FFFFFF">
                                                <?php echo $row_product['rightlink1_heading'];?>                                            </td>
                                            <td width="56%"  bgcolor="#FFFFFF"><?php echo $row_product['rightlink1_details'];?></td>
                                            <td width="8%"  bgcolor="#FFFFFF"><a href="add_new_right_link1.php?action=edit&rightlink1_id=<?php echo $row_product['rightlink1_id']?>"><img src="images/Edit.gif" width="12" height="12" alt="Edit" border="0" /></a></td>
                                            <td width="8%"   bgcolor="#FFFFFF"><a href="view_right_link1.php?action=delete&rightlink1_id=<?php echo $row_product['rightlink1_id']?>"  onClick=" javascript: return confirm('Are you sure You want to delete ?');"><img src="images/del.gif" width="12" height="10" border="0" /></a> </td>
                                          </tr>
                                          <?php }
											}else{
											?>
									...
									<!-- Paging ends starts here  -->	
                                          ...
                                          <tr>
                                            <td  height="33" colspan="15" align="center" bgcolor="#3c7701"><input name="add" type="button" value="Add New Record"  onClick="window.location='add_new_right_link1.php';" /></td>
                                          </tr>
                                          <!-----------------------End your loop here---------------------------->
                                        </table>
                </form></td>
